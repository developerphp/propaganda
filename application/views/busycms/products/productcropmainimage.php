...
<body>
    ...
    <div id="dashboard">
        <?php 
                $sql=$this->db->query("select * from products where id=".$id." order by id asc");
                foreach($sql->result() as $image) {  ?>
        <div class="scroll con eventslist">
            <div class="top-title" style="height:30px;overflow:hidden;">    
                <div class="detail-title">
                <form id="cropimageformproduct" method="post" onSubmit="return false;">
                        ...
                        <button onclick="submitform('busycms/','cropimageformproduct')">Crop Image</button>
                        </div>
                    </form>
                    <button style="position:absolute;left:0;right:auto;" onclick="location.href='<?php echo base_url() ?>busycms/editproduct/<?php echo $id ?>'">
                        Back
                    </button>
                </div>
            </div>
            <div id="cropimageformproduct_back"></div>
